  ...
  module Activate
    ...
    def run(test_run, confirmation_token, fixture_name = nil)
      ...
      if response.code == 200
        ...
        test_run.logger.debug "Member #{response_hash[:uuid]} activated"
...
begin
  unless ARGV.length.between?(1, 2)
    puts "Usage: ruby activate.rb confirmation_token fixture_name\n"
    puts " 'confirmation_token' is the confirmation token from the sign-up email\n"
    puts " you can get that by tailing the mailer service log, or the member_service log\n\n"
    exit
  end

  test_run = Test.new('activate', Logger::DEBUG)
  test_run.start

  confirmation_token = ARGV[0]
  fixture_name = ARGV[1]
  test_run.logger.debug "Activating with '#{confirmation_token}'"
  test_run.logger.debug "Using fixture: #{fixture_name}" if fixture_name
  test_run.logger.debug ''
  results = Tests::Activate.run(test_run, confirmation_token, fixture_name)
