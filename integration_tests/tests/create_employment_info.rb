...
module Tests
  ...
  module CreateEmploymentInfo
    ...
    def run(test_run, username, password, credit_application_id, fixture_name = nil)
      ...
      results = acct.create_employment_info(credit_application_id, body)
    end

    private 

      def fixture(fixture_name)
        fixture_name ? Fixtures::EmploymentInfo.send(fixture_name) : Fixtures::EmploymentInfo::valid_data
      end
  end
end
...
begin
  unless ARGV.length.between?(3, 4)
    puts "Usage: ruby create_employment_info.rb username password credit_application_id fixture_name\n"
    exit
  end

  test_run = Test.new('create-employment-details', Logger::DEBUG)
  test_run.start
  ...
  results = Tests::CreateEmploymentInfo.run(test_run, username, password, credit_application_id, fixture_name)
