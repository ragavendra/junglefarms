...
module Tests
  module CreateContactNumber
    ...
    def run(test_run, username, password, credit_application_id, fixture_name = nil)
      ...
      test_run.logger.debug "Creating contact number for application with id: #{credit_application_id}"
      body = fixture(fixture_name)
      results = acct.create_contact_number(credit_application_id, body)
    end
...
begin
  unless ARGV.length.between?(3, 4)
    puts "Usage: ruby create_contact_number.rb username password application_id fixture_name\n"
    exit
  end

  test_run = Test.new('create-contact-number', Logger::DEBUG)
  test_run.start
  ...
  results = Tests::CreateContactNumber.run(test_run, username, password, application_id, fixture_name)
